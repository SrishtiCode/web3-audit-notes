...
Copy this block and fill it in for each finding you read.
...
function getPrice() public view returns (uint) {
    return ICurvePool(pool).get_virtual_price(); // stale during reentrant call
}
```
 
**fix:**
Add a nonReentrant modifier to any function that reads from external pools, or use a Chainlink feed as fallback.
 
**my takeaway:**
Always check whether view functions that read external state are called inside an execution path that has open external calls before them. Read-only reentrancy is easy to miss because devs assume views are harmless.